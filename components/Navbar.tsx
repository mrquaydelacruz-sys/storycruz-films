...
export default function Navbar({ logoUrl }: { logoUrl?: string }) {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-[100] transition-transform duration-500 ease-in-out ${isVisible ? 'translate-y-0' : '-translate-y-full'
        }`}
    >
      ...
      <div className={`fixed inset-0 bg-black z-[110] transition-all duration-500 ease-in-out flex flex-col items-center justify-center gap-10 ${isOpen ? 'opacity-100 visible pointer-events-auto' : 'opacity-0 invisible pointer-events-none'
        }`}>
        {links.map((link) => (
          <Link
            key={link.name}
            ...
            className={`text-3xl uppercase tracking-[0.4em] font-serif transition-colors hover:text-accent ${pathname === link.href ? 'text-accent' : 'text-white'
              }`}
          >
            {link.name}
          </Link>
